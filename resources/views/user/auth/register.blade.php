								...
								<form class="card-body border-top-0 pt-3" method="post" action="{{route('auth.register')}}">
									...
									<div class="form-group @error('agree_terms') is-invalid @enderror">
										<label class="custom-control form-checkbox">
											...
											<span class="custom-control-label">{{trans('langconvert.userdashboard.iagree')}} <a href="{{setting('terms_url')}}" class="text-primary">
													{{trans('langconvert.userdashboard.termsservices')}}</a></span>
										</label>
									</div>
									@error('agree_terms')
